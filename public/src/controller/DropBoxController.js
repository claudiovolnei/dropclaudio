class DropBoxController {
    ...
    connectFirebase() {
        var firebaseConfig = {
            apiKey: "AIzaSyAZQS7PCyZzMzuHLmAMfNv6_vz05up4swQ",
            authDomain: "dropbox-clone-9bbab.firebaseapp.com",
            databaseURL: "https://dropbox-clone-9bbab.firebaseio.com",
            projectId: "dropbox-clone-9bbab",
            storageBucket: "dropbox-clone-9bbab.appspot.com",
            messagingSenderId: "868288448381",
            appId: "1:868288448381:web:e4476b0cd7a4139adb6e14",
            measurementId: "G-2JZE9RCK49"
          };
    ...
    removeFolderTask(ref, name) {
        return new Promise((resolve, reject) => {
            ...
            folderRef.on('value', snapashot => {
                folderRef.off('value');
                snapashot.forEach(item => {
                    let data = item.val();
                    data.key = item.key;
                    if(data.type === 'folder') {
        ...
        [...files].forEach(file => {
           ...
           promises.push(new Promise((resolve, reject) => {
            ...
            task.on('state_changed', snapshot => {
                ...
                this.urlStorage = fileRef.getDownloadURL();
    ...
    renderNav() {
        ...
        for (let i = 0; i < this.currentFolder.length; i++) {
            ...
            if((i + 1) === this.currentFolder.length) {
                ...
                <svg width="24" height="24" viewBox="0 0 24 24" class="mc-icon-template-stateless" style="top: 0px; position: relative;">
